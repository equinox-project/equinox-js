...
const streamVersion = (evt: TimelineEvent) => {
  const meta = evt.metadata as Meta | null
...
export const decode = <V>(
  tryDecode: (ev: TimelineEvent) => V | null | undefined,
  events: TimelineEvent[]
): [StreamToken, V] | null => {
  if (events.length > 0) {
    const decoded = tryDecode(events[0])
    if (decoded != null)
      return [Token.create(streamVersion(events[0])), decoded]
  }
  return null
}
