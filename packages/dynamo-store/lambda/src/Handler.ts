...
const parse = (records: DynamoDBStreamEvent["Records"]): AppendsEpoch.Events.StreamSpan[] => {
  ...
  try {
    for (const record of records) {
      if (
        record.dynamodb?.StreamViewType !== "NEW_IMAGE" &&
        record.dynamodb?.StreamViewType !== "NEW_AND_OLD_IMAGES"
      ) {
      ...
      switch (record.eventName) {
        ...
        case "MODIFY": {
          ...
          if (p[0] === "$") indexStream++
            ...
            const appendedLen = updated.a != null ? Number(updated.a.N) : 0
              ...
              if (appendedEventTypes.length > 0) {
                ...
                const et =
                  appendedEventTypes.length === 1
                    ? ":" + appendedEventTypes[0]
                    : `${appendedEventTypes[0]}+${appendedEventTypes.length - 1}`
