...
export const create = (category: string, streamId: string) =>
  Internal.ofCategoryAndStreamId(category, streamId)
export const compose = (category: string, streamIds: string[]) =>
  Internal.ofCategoryAndStreamId(category, Internal.createStreamId(streamIds))
...
export const parse = (streamName: string) => {
  if (!streamName.includes("-"))
    throw new Error(`StreamName ${streamName} must include a "-" separator`)
