...
const codec = {
  encode: (ev: Event): StreamEvent => ev as StreamEvent,
  decode: (ev: TimelineEvent) => ev as Event,
}
    ...
    case "Completed":
      return {
        ...
        items: state.items.map((x) =>
          x.id === event.data.id ? {...x, completed: true}: x
        ),
      }
    case "Deleted":
      return {
  ...
  delete:
    ...
    (state: State): Event[] => {
      if (state.items.some((x) => x.id === id))
        return [{ type: "Deleted", data: { id } }]
      return []
    },
  clear: (state: State): Event[] =>
    state.items.length > 0 ? [{ type: "Cleared" }] : [],
}
...
export class Service {
  ...
  static build(context: MessageDbContext, cache: CachingStrategy) {
    const category = MessageDbCategory.build<Event, State, null>(
      context,
      codec,
      fold,
      initial,
    ...
    const resolve = Decider.resolve<Event, State, null>(category)(Category)
    return new Service((id) => resolve(id, null))
  }
}
