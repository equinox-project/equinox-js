...
The example below shows the default shape we recommend: explicit events and
folding logic, decision functions per operation, and a small `Service` boundary
...
export class Service {
  constructor(
    private readonly resolve: (accountId: AccountId) => Decider<Events.Event, Fold.State>,
  ) {}
