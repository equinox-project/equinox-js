...
appending events to a stream. Single-stream consistency is the core guarantee,
and cross-stream work is expressed explicitly through reactions and process
managers rather than hidden in ambient transactions. In general this is done in
a few steps:
