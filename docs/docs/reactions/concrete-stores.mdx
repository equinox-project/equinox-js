...
`Sink`s rather than handler functions so we must first construct a sink wrapping
the handler.
...
`create`. When passed one thing differs from normal operation: MessageDB will
filter messages based on a hash of the stream id.

This means that each consumer in the group needs to maintain a distinct
checkpoint and is therefore not easily autoscaled. In order to expand or
contract a consumer group you will need to wipe the checkpoints and have all of
the consumers in the group start from the current lowest checkpoint.

You're in control of the `groupName` and should ensure that it is unique across
all consumers in the group (e.g. by appending the `consumerGroupMember` to the
group name). This is because the group name is used for checkpointing.
